...
The existing mock scheduler and its PD/continuous-batching tests remain separate. `AscendBackend` and registered hardware transport are still unimplemented. Native continuous batching and TP=2 are now qualified; see [batching](npu-continuous-batching.md) and [TP](npu-tensor-parallel.md). Native PD, a persistent compiled native cache and performance parity remain unimplemented.
