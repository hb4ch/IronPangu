...
cargo build --release --manifest-path "$root/Cargo.toml" --locked --target aarch64-unknown-linux-gnu -p pangu-cli
mkdir -p "$root/.deploy"
target_dir=$(cargo metadata --manifest-path "$root/Cargo.toml" --format-version 1 --no-deps | sed -n 's/.*"target_directory":"\([^"]*\)".*/\1/p')
aarch64-linux-gnu-strip -o "$root/.deploy/pangu-npu" "$target_dir/aarch64-unknown-linux-gnu/release/pangu"
