...
mod error;
mod backend;
pub use backend::{GenerationBackend, BackendMetadata};
mod inflight;
mod log_stats;
mod output;
mod request;
...
pub struct Llm {
    ...
    backend: Option<std::sync::Arc<dyn GenerationBackend>>,
...
impl Llm {
    ...
    pub fn with_log_stats(mut self, enabled: bool) -> Self {
        if enabled && self.client.is_some() {
            let stats_logger = StatsLogger::start(
                self.client.as_ref().expect("checked client").model_name().to_string(),
                self.client.as_ref().expect("checked client").engine_indices(),
            );
    ...
    /// calls.
    pub fn engine_core_client(&self) -> Option<&EngineCoreClient> {
        self.client.as_ref()
    }

    /// Construct a frontend facade without an engine-core connection.
    pub fn from_backend(backend: std::sync::Arc<dyn GenerationBackend>) -> Self {
        Self { client: None, backend: Some(backend), randomize_request_id: false,
    ...
    pub fn metadata(&self) -> BackendMetadata {
        ...
        BackendMetadata { max_model_len: client.max_model_len(), model_dtype: client.model_dtype(),
            version: client.vllm_version().into(), healthy: client.is_healthy() }
    }
    ...
    pub async fn generate(&self, req: GenerateRequest) -> Result<GenerateOutputStream> {
        if let Some(backend) = &self.backend { return backend.generate(req).await; }
        ...
        let stream = self.client.as_ref().expect("checked client").call(prepared.engine_request).await?;
    ...
    pub async fn abort(&self, external_ids: &[String]) -> Result<()> {
        if let Some(backend) = &self.backend { return backend.abort(external_ids).await; }
        let internal_ids = self.inflight.resolve(external_ids);
        if internal_ids.is_empty() {
            return Ok(());
        }
        self.client.as_ref().expect("checked client").abort(&internal_ids).await?;
        Ok(())
    }

    /// Shut down the underlying engine-core client and its background tasks.
    pub async fn shutdown(self) -> Result<()> {
        if let Some(backend) = self.backend { return backend.shutdown().await; }
        if let Some(client) = self.client { client.shutdown().await?; }
        Ok(())
    }
}
