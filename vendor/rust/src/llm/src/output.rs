...
pub struct EngineGenerateOutputStream {
...
impl EngineGenerateOutputStream {
...
impl Stream for EngineGenerateOutputStream {
...
impl FusedStream for EngineGenerateOutputStream {
    fn is_terminated(&self) -> bool {
        self.raw_stream.is_terminated()
    }
}

impl Drop for EngineGenerateOutputStream {
    fn drop(&mut self) {
...
/// Backend-neutral stream. The owned inner stream handles cancellation on drop.
pub struct GenerateOutputStream {
    request_id: String,
    inner: Pin<Box<dyn Stream<Item = Result<GenerateOutput>> + Send>>,
    ended: bool,
}
impl GenerateOutputStream {
    pub fn from_stream(request_id: String, inner: impl Stream<Item = Result<GenerateOutput>> + Send + 'static) -> Self {
        Self { request_id, inner: Box::pin(inner), ended: false }
    }
    pub(crate) fn new(prompt_token_ids: Arc<[u32]>, raw_stream: EngineCoreOutputStream,
        request_metrics: RequestMetricsTracker, request_guard: RequestGuard) -> Self {
        let stream = EngineGenerateOutputStream::new(prompt_token_ids, raw_stream, request_metrics, request_guard);
        Self::from_stream(stream.request_id().into(), stream)
    }
...
impl Stream for GenerateOutputStream {
    ...
    fn poll_next(mut self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Option<Self::Item>> {
        ...
        if let Poll::Ready(value) = &item {
            match value {
                Some(Ok(output)) if output.finished() => self.ended = true,
                Some(Err(_)) => self.ended = true,
                None => {
                    self.ended = true;
                    return Poll::Ready(Some(Err(crate::Error::Backend {
                        message: "generation stream closed without terminal output".into() })));
                }
                _ => {}
            }
        }
...
impl FusedStream for GenerateOutputStream {
    fn is_terminated(&self) -> bool { self.ended }
}
