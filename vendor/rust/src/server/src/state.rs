...
impl AppState {
    ...
    pub async fn load_lora(
        ...
        self.lora_manager
            .load_lora(
                self.chat.engine_core_client().ok_or_else(|| LoadLoraError::Engine(vllm_engine_core_client::Error::UnsupportedField { field: "lora", context: "in-process backend" }))?,
                &self.served_model_names,
                lora_name,
                lora_path,
                load_inplace,
    ...
    pub async fn unload_lora(
        ...
        self.lora_manager
            .unload_lora(self.chat.engine_core_client().ok_or_else(|| UnloadLoraError::Engine(vllm_engine_core_client::Error::UnsupportedField { field: "lora", context: "in-process backend" }))?, lora_name, lora_int_id)
            .await
    }

    /// Return a reference to the underlying engine core client for utility
    /// calls.
    pub(crate) fn engine_core_client(&self) -> Result<&EngineCoreClient, crate::error::ApiError> {
        self.chat.engine_core_client().ok_or_else(|| crate::error::ApiError::InvalidRequest {
            message: "engine-core administration is unavailable for this backend".into(), param: None })
    }
