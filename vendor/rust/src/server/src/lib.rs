...
/// client.
async fn build_state(config: &Config, backend: Option<Arc<dyn vllm_llm::GenerationBackend>>) -> Result<Arc<AppState>> {
    ...
    let llm = if let Some(backend) = backend { Llm::from_backend(backend) } else {
    ...
    Llm::new(client).with_log_stats(!config.disable_log_stats)
    };
...
{
    serve_inner(config, shutdown, extend_router, None).await
}

/// Serve the existing vLLM Rust routes using an in-process generation backend.
pub async fn serve_with_backend(config: Config, shutdown: CancellationToken,
    backend: Arc<dyn vllm_llm::GenerationBackend>) -> Result<()> {
    serve_inner(config, shutdown, |router| router, Some(backend)).await
}

async fn serve_inner<F>(config: Config, shutdown: CancellationToken, extend_router: F,
    backend: Option<Arc<dyn vllm_llm::GenerationBackend>>) -> Result<()>
where F: FnOnce(Router) -> Router {
    ...
    let state = tokio::select! {
        result = build_state(&config, backend) => result?,
