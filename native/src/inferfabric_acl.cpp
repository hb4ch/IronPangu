...
struct inferfabric_acl_session {
  ...
  bool dedicated_activations = [] {
    const char *mode = std::getenv("INFERFABRIC_DEDICATED_ACTIVATIONS");
  ...
  bool dedicated_workspaces = [] {
    const char *mode = std::getenv("INFERFABRIC_DEDICATED_WORKSPACES");
    return mode && std::string(mode) == "1";
  }();
  ...
  struct Linear {
    ...
    inferfabric_acl_session *owner = nullptr;
...
// are retained.
struct Composite {
  inferfabric_acl_session *session;
  std::unique_ptr<inferfabric_acl_session::Linear> op =
      std::make_unique<inferfabric_acl_session::Linear>();
  explicit Composite(inferfabric_acl_session *s) : session(s) {
    s->enter();
    op->owner = s;
  }
  ...
  template <class F>
  void step(inferfabric_acl_session::Linear::Launch launch, F query,
            const char *name) {
...
extern "C" {
uint32_t inferfabric_acl_abi_version(void) { return 2; }
const char *inferfabric_acl_last_error(void) { return last_error.c_str(); }
int32_t inferfabric_acl_open(int32_t device, inferfabric_acl_session **out) {
  return boundary([&] {
    ...
    *out = nullptr;
    ...
    auto *s = new inferfabric_acl_session;
    try {
...
int32_t inferfabric_acl_set_memory_budget(inferfabric_acl_session *s, uint64_t bytes,
                                    uint64_t reserve) {
  return boundary([&] {
    if (!s || !bytes)
...
int32_t inferfabric_acl_memory_snapshot(inferfabric_acl_session *s,
                                  inferfabric_acl_memory_stats *out) {
  return boundary([&] {
    if (!s || !out)
    ...
    *out = {};
    ...
    out->buffer_bytes += inferfabric_acl_session::pool_bytes(s->layer_pool);
    out->temporary_bytes += inferfabric_acl_session::pool_bytes(s->scratch_pool) +
                            inferfabric_acl_session::pool_bytes(s->snapshot_pool);
    s->check_allocation(0);
  });
}
int32_t inferfabric_acl_allocation_snapshot(inferfabric_acl_session *s,
                                      inferfabric_acl_allocation_stats *out) {
  return boundary([&] {
    if (!s || !out)
      throw std::runtime_error("invalid allocation snapshot");
    s->enter();
    *out = {};
    out->scratch_pool_bytes = inferfabric_acl_session::pool_bytes(s->scratch_pool);
    out->layer_pool_bytes = inferfabric_acl_session::pool_bytes(s->layer_pool);
    out->snapshot_pool_bytes = inferfabric_acl_session::pool_bytes(s->snapshot_pool);
...
int32_t inferfabric_acl_buffer_view(inferfabric_acl_session *s, uint64_t parent,
  ...
  return boundary([&] {
    ...
    auto id = s->next++;
    s->buffers.emplace(id, inferfabric_acl_session::Buffer{ptr, bytes, false});
    *handle = id;
  });
}
int32_t inferfabric_acl_allocate(inferfabric_acl_session *s, uint64_t bytes,
                           uint64_t *handle) {
  return boundary([&] {
    if (!s || !handle || bytes == 0 ||
    ...
    try {
      auto id = s->next++;
      s->buffers.emplace(id, inferfabric_acl_session::Buffer{ptr, bytes});
      *handle = id;
      if (s->memory_budget)
        s->observe_memory();
    } catch (...) {
...
int32_t inferfabric_acl_write(inferfabric_acl_session *s, uint64_t id, uint64_t offset,
...
int32_t inferfabric_acl_read(inferfabric_acl_session *s, uint64_t id, uint64_t offset,
...
int32_t inferfabric_acl_capture_copy(inferfabric_acl_session *s, uint64_t source,
...
int32_t inferfabric_acl_replay(inferfabric_acl_session *s, uint64_t graph) {
  return boundary([&] {
    if (!s)
...
int32_t inferfabric_acl_device_count(uint32_t *count) {
  return boundary([&] {
    if (!count)
...
uint64_t inferfabric_acl_tp_root_size(void) { return sizeof(HcclRootInfo); }
int32_t inferfabric_acl_tp_root(void *bytes, uint64_t size) {
  return boundary([&] {
...
int32_t inferfabric_acl_tp_init(inferfabric_acl_session *s, uint32_t world, uint32_t rank,
...
int32_t inferfabric_acl_linear_prepare(inferfabric_acl_session *s, uint64_t x, uint64_t w,
  ...
  return boundary([&] {
    ...
    s->enter();
    auto op = std::make_unique<inferfabric_acl_session::Linear>();
    op->owner = s;
...
int32_t inferfabric_acl_operation_execute(inferfabric_acl_session *s, uint64_t operation) {
  return boundary([&] {
    if (!s)
      throw std::runtime_error("null session");
    s->enter();
...
int32_t inferfabric_acl_operation_capture(inferfabric_acl_session *s, uint64_t operation,
                                    uint64_t *graph) {
  return boundary([&] {
    if (!s || !graph)
...
int32_t inferfabric_acl_linear_execute(inferfabric_acl_session *s, uint64_t id) {
  return inferfabric_acl_operation_execute(s, id);
}
int32_t inferfabric_acl_linear_capture(inferfabric_acl_session *s, uint64_t id,
                                 uint64_t *graph) {
  return inferfabric_acl_operation_capture(s, id, graph);
}
int32_t inferfabric_acl_rms_prepare(inferfabric_acl_session *s, uint64_t x, uint64_t gamma,
...
int32_t inferfabric_acl_delta_prepare(inferfabric_acl_session *s, uint64_t q, uint64_t k,
...
int32_t inferfabric_acl_conv_prepare(inferfabric_acl_session *s, uint64_t x,
...
int32_t inferfabric_acl_sequence_prepare(inferfabric_acl_session *s, const uint64_t *ids,
...
int32_t inferfabric_acl_pointwise_prepare(inferfabric_acl_session *s, uint64_t a,
...
int32_t inferfabric_acl_delta_transforms_prepare(inferfabric_acl_session *s, uint64_t conv,
...
int32_t inferfabric_acl_gated_rms_prepare(inferfabric_acl_session *s, uint64_t x,
...
int32_t inferfabric_acl_attention_qk_prepare(inferfabric_acl_session *s, uint64_t packed_q,
...
int32_t inferfabric_acl_paged_attention_prepare(
    inferfabric_acl_session *s, uint64_t q, uint64_t k, uint64_t v, uint64_t gate,
...
int32_t inferfabric_acl_embedding_prepare(inferfabric_acl_session *s, uint64_t weight,
...
int32_t inferfabric_acl_model_layer_prepare(
    inferfabric_acl_session *s, int32_t kind, const uint64_t *w, uint64_t count,
  ...
  return boundary([&] {
    ...
    auto alloc = [&](uint64_t bytes) {
      uint64_t id = 0;
      checked(inferfabric_acl_allocate(s, bytes, &id));
      return id;
    };
    ...
    auto linear = [&](uint64_t x, uint64_t weight, uint64_t y, int64_t n,
                      int64_t k) {
      emit([&](auto *id) {
        return inferfabric_acl_linear_prepare(s, x, weight, y, 1, n, k, id);
      });
    };
    auto point = [&](uint64_t a, uint64_t b, uint64_t y, int64_t n,
                     int32_t mode) {
      emit([&](auto *id) {
        return inferfabric_acl_pointwise_prepare(s, a, b, y, n, mode, id);
      });
    };
    auto normed = alloc(2048 * 2);
    emit([&](auto *id) {
      return inferfabric_acl_rms_prepare(s, input, w[0], normed, 1, 2048, id);
    });
    ...
    if (kind == 0) {
      ...
      *state_b = alloc(16 * 128 * 128 * 4);
      ...
      emit([&](auto *id) {
        return inferfabric_acl_conv_prepare(s, qkv, w[9], *state_a, conv, 1, 6144,
                                      id);
      });
      emit([&](auto *id) {
        return inferfabric_acl_delta_transforms_prepare(s, conv, a, b, w[10], w[11],
                                                  q, k, v, g, beta, 1, id);
      });
      emit([&](auto *id) {
        return inferfabric_acl_delta_prepare(s, q, k, v, g, beta, *state_b, core, 16,
                                       128, 128, id);
      });
      emit([&](auto *id) {
        return inferfabric_acl_gated_rms_prepare(s, core, z, w[12], gated, 16, 128,
                                           id);
      });
      ...
      *state_b = alloc(slots * 512 * 2);
      ...
      emit([&](auto *id) {
        return inferfabric_acl_attention_qk_prepare(s, qp, ki, w[9], w[10], positions,
                                              q, k, gate, 1, id);
      });
      emit([&](auto *id) {
        return inferfabric_acl_paged_attention_prepare(s, q, k, vi, gate, *state_a,
    ...
    emit([&](auto *id) {
      return inferfabric_acl_rms_prepare(s, residual, w[1], ff, 1, 2048, id);
    });
    ...
    checked(inferfabric_acl_sequence_prepare(s, ops.data(), ops.size(), operation));
  });
}
int32_t inferfabric_acl_embedding_batch_prepare(inferfabric_acl_session *s, uint64_t weight,
...
int32_t inferfabric_acl_model_layer_batch_prepare(
    inferfabric_acl_session *s, int32_t kind, const uint64_t *w, uint64_t count,
    uint64_t input, uint64_t output, uint64_t positions, uint64_t append,
    uint64_t table, uint64_t mask, int64_t slots, int64_t context,
  ...
  return boundary([&] {
    ...
    auto persistent = [&](uint64_t bytes) {
      uint64_t id = 0;
      checked(inferfabric_acl_allocate(s, bytes * batch, &id));
      return id;
    };
    ...
    auto alloc = [&](uint64_t bytes) {
      ...
      s->buffers.emplace(id,
                         inferfabric_acl_session::Buffer{ptr, bytes * batch, false});
      return id;
    };
    auto emit = [&](auto build) {
      uint64_t id = 0;
    ...
    auto linear = [&](uint64_t x, uint64_t weight, uint64_t y, int64_t n,
                      int64_t k) {
      emit([&](auto *id) {
        return inferfabric_acl_linear_prepare(s, x, weight, y, batch, n, k, id);
      });
    };
    auto point = [&](uint64_t a, uint64_t b, uint64_t y, int64_t n,
                     int32_t mode) {
      emit([&](auto *id) {
        return inferfabric_acl_pointwise_prepare(s, a, b, y, batch * n, mode, id);
      });
    };
    ...
    auto normed = alloc(2048 * 2);
    emit([&](auto *id) {
      return inferfabric_acl_rms_prepare(s, input, w[0], normed, batch, 2048, id);
    });
    auto attn = alloc(2048 * 2);
    if (kind == 0) {
      auto qkv = alloc(6144 * 2), z = alloc(2048 * 2), a = alloc(16 * 2),
      ...
      *state_b = persistent(16 * 128 * 128 * 4);
      ...
      linear(normed, w[8], b, 16, 2048);
      emit([&](auto *id) {
        return inferfabric_acl_conv_prepare(s, qkv, w[9], *state_a, conv, batch, 6144,
                                      id);
      });
      emit([&](auto *id) {
        return inferfabric_acl_delta_transforms_prepare(s, conv, a, b, w[10], w[11],
      ...
      emit([&](auto *id) {
        return inferfabric_acl_delta_prepare(s, q, k, v, g, beta, *state_b, core,
                                       batch * 16, 128, 128, id);
      });
      emit([&](auto *id) {
        return inferfabric_acl_gated_rms_prepare(s, core, z, w[12], gated, batch * 16,
                                           128, id);
      });
      linear(gated, w[13], attn, 2048, 2048);
    } else {
      ...
      *state_b = persistent((slots / batch) * 512 * 2);
      ...
      emit([&](auto *id) {
        return inferfabric_acl_paged_attention_prepare(
    ...
    point(input, attn, residual, 2048, 0);
    emit([&](auto *id) {
      return inferfabric_acl_rms_prepare(s, residual, w[1], ff, batch, 2048, id);
    });
    linear(ff, w[2], gate, 6144, 2048);
    linear(ff, w[3], up, 6144, 2048);
    point(gate, 0, act, 6144, 2);
...
int32_t inferfabric_acl_copy_region(inferfabric_acl_session *s, uint64_t src,
...
int32_t inferfabric_acl_close(inferfabric_acl_session *s) {
  return boundary([&] {
    if (!s)
      return;
