...
An inference compiler and runtime for Huawei Ascend, built in Rust with a C ABI to C++/Ascend C operators and CANN libraries. Model structure and parallel execution are declared in a DSL. Serving uses the **vLLM 0.25.1 Rust frontend**, backed by Iron Pangu's scheduler and native execution engine.
...
Iron Pangu takes that approach: Rust owns compilation, checkpoint loading, scheduling, and resource lifetimes; C-compatible operator interfaces provide the device computations. The current implementation uses C++ to call ACL/ACLNN and HCCL, with Ascend C as the path for custom kernels. Removing PyTorch means taking responsibility for operator semantics, activation storage, asynchronous execution, and numerical validation. Those are explicit parts of this project.
...
For example, these declarations come from the [complete TP2 program](examples/qwen35-2b-tp2.pangu):
...
Today, the C++ bridge is compiled ahead of time, and the native runtime prepares vendor operators and captures graphs at startup. Serialized `.pangu-bin` artifacts contain versioned, checksummed plans; they are not standalone native model executables. Whole-program native code generation and JIT kernel specialization are roadmap items. ACL Graph capture itself is not a substitute for either.
...
These measurements compare Iron Pangu allocator configurations, not Iron Pangu against PyTorch or another serving engine. See [activation memory validation](docs/npu-activation-memory.md) for reports, methodology, and limits.
...
Deploy `.deploy/pangu-server` as `pangu-native-server`. In the NPU container, from the repository directory:
...
curl http://127.0.0.1:18081/v1/chat/completions \
  ...
  -d '{"model":"ironpangu-qwen35","messages":[{"role":"user","content":"What is 2 + 2?"}],"temperature":0,"max_tokens":16}'
```

For CPU-only development, `cargo run -p pangu-cli -- demo examples/qwen35-2b.pangu examples/requests.json` exercises the deterministic mock compiler/scheduler. Mock communication traces are not hardware qualification.
...
|---|---|
| `crates/pangu-dsl`, `pangu-model`, `pangu-ir` | Language, model contracts, typed execution/state metadata |
| `crates/pangu-compiler` | Checkpoint validation, lowering, specialization, artifact integrity and cache |
| `crates/pangu-native`, `native/` | Native model runner, activation planning, ACL/ACLNN graphs, HCCL, sampling |
| `crates/pangu-scheduler`, `pangu-runtime`, `pangu-transfer` | Scheduling and runtime/transfer contracts, including mock distributed qualification |
| `frontend/`, `vendor/rust/` | Iron Pangu backend integration and pinned vLLM Rust frontend |
| `crates/pangu-cli`, `scripts/` | Compiler/probe commands, local cross-compilation, deployment and validation |
