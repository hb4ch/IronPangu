...
use super::*;
use inferfabric_compiler::lower::{Binding, Transform};
...
pub fn run_model(
    ...
    let checkpoint = inferfabric_compiler::checkpoint::inspect(dir)?;
    let plan = inferfabric_compiler::bound::compile(&spec, checkpoint)?;
    let context = spec.context_buckets[0];
    let program = inferfabric_compiler::lower::lower(&plan, context)?;
    if tokens.is_empty()
        || tokens.len() > context
    ...
    unsafe {
        let embed: Embedding = *api
            ._library
            .get(b"inferfabric_acl_embedding_prepare\0")
            .map_err(|e| invalid(e.to_string()))?;
        let block: Block = *api
            ._library
            .get(b"inferfabric_acl_model_layer_prepare\0")
            .map_err(|e| invalid(e.to_string()))?;
        ...
        for item in &program.blocks {
            ...
            let mut b = 0;
            let mut op = 0;
            let is_full = item.kind == inferfabric_model::Layer::Full;
            api.check(block(
                s.ptr,
                i32::from(is_full),
                bindings.as_ptr(),
