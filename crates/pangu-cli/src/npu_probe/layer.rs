...
pub fn run_layer(checkpoint_dir: &Path, library: &Path, device: i32) -> Result<serde_json::Value> {
    let checkpoint = pangu_compiler::checkpoint::inspect(checkpoint_dir)?;
    ...
    unsafe {
        ...
        let conv_fn: Conv = *api
            ._library
            .get(b"pangu_acl_conv_prepare\0")
            .map_err(|e| invalid(e.to_string()))?;
        let transform: Transform = *api
            ._library
            .get(b"pangu_acl_delta_transforms_prepare\0")
            .map_err(|e| invalid(e.to_string()))?;
        let gated_fn: Gated = *api
            ._library
            .get(b"pangu_acl_gated_rms_prepare\0")
            .map_err(|e| invalid(e.to_string()))?;
        let pw: Point = *api
            ._library
            .get(b"pangu_acl_pointwise_prepare\0")
            .map_err(|e| invalid(e.to_string()))?;
