...
impl StartupOptions {
    pub fn validate(&self) -> Result<()> {
        ...
        if self.max_model_len == 0 || self.max_model_len > pangu_compiler::checkpoint::MAX_MODEL_LEN
        {
            return Err(invalid(
...
impl MemoryProfile {
    pub(crate) fn start(
        ...
        let set: SetBudget = unsafe {
            *s.api
                ._library
                .get(b"pangu_acl_set_memory_budget\0")
    ...
    pub(crate) fn record(&mut self, s: &Session<'_>, name: &str) -> Result<MemoryStats> {
        let query: Snapshot = unsafe {
            *s.api
                ._library
                .get(b"pangu_acl_memory_snapshot\0")
                .map_err(|e| invalid(e.to_string()))?
        };
    ...
    fn save(&self) -> Result<()> {
        if let Some(path) = &self.path {
            ...
            pangu_compiler::write_atomic(path, &data)?;
        }
        Ok(())
    }
}
pub(crate) fn save_failure(options: &StartupOptions, message: &str) {
    if let Some(path) = &options.profile_path {
        ...
        if let Ok(bytes) = serde_json::to_vec_pretty(&value) {
            let _ = pangu_compiler::write_atomic(path, &bytes);
        }
    }
}
#[cfg(test)]
