...
use inferfabric_ir::*;
use inferfabric_model::{Layer, Result, Role, Spec, invalid};
...
// Keep the original format marker so existing serialized plans remain readable.
...
pub fn key(spec: &Spec, target: &Target) -> Result<String> {
    ...
    digest.update(include_bytes!("../../inferfabric-ir/src/lib.rs"));
    digest.update(include_bytes!("../../inferfabric-model/src/lib.rs"));
    Ok(digest
        .finalize()
        .iter()
        .map(|b| format!("{b:02x}"))
...
pub fn cached(spec: &Spec, target: &Target, dir: &Path) -> Result<(Artifact, bool)> {
    let path = dir.join(format!("{}.inferfabric-bin", key(spec, target)?));
    match fs::read(&path) {
        Ok(bytes) => {
            let a = decode(&bytes)?;
...
mod tests {
    use super::*;
    fn spec() -> Spec {
        inferfabric_dsl::parse(include_str!("../../../examples/qwen35-2b.inferfabric")).unwrap()
    }
    #[test]
...
mod cache_tests {
    ...
    fn cache_roundtrip_miss_hit_and_invalidation() {
        ...
        let path = dir.join(format!("{}.inferfabric-bin", b.key));
