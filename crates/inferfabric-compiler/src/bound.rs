...
use inferfabric_model::{Layer, Result, Spec, invalid};
...
pub fn compile(spec: &Spec, checkpoint: Checkpoint) -> Result<Plan> {
    ...
    let mut plan=Plan {
        format:"inferfabric.typed-plan.v1".into(),executable:false,contract:"qwen35_text_v1".into(),
...
mod bound_tests {
    ...
    fn spec() -> Spec {
        inferfabric_dsl::parse_checkpoint(include_str!(
            "../../../examples/qwen35-2b-checkpoint.inferfabric"
        ))
        .unwrap()
    }
    #[test]
