...
impl<'a> Engine<'a> {
    fn prepare(
        ...
        let program = inferfabric_compiler::lower::lower_batch(plan, context, batch)?;
        ...
        unsafe {
            let embed: Embedding = *api
                ._library
                .get(b"inferfabric_acl_embedding_batch_prepare\0")
                .map_err(|e| invalid(e.to_string()))?;
            let block: Block = *api
                ._library
                .get(b"inferfabric_acl_model_layer_batch_prepare\0")
                .map_err(|e| invalid(e.to_string()))?;
            ...
            for (layer_index, item) in program.blocks.iter().enumerate() {
                ...
                let mut b = 0;
                let mut op = 0;
                let full = item.kind == inferfabric_model::Layer::Full;
                api.check(block(
                    s.ptr,
                    i32::from(full),
                    bindings.as_ptr(),
    ...
    pub fn sample_slot(&mut self, lane: usize) -> Result<u32> {
        ...
        let copy: CopyRegion = unsafe {
            *self
                ...
                .api
                ._library
                .get(b"inferfabric_acl_copy_region\0")
                .map_err(|e| invalid(e.to_string()))?
        };
        self.session.api.check(unsafe {
            copy(
