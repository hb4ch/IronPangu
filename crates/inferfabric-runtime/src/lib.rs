...
use inferfabric_ir::*;
use inferfabric_model::{Error, Layer, Result, Role, Spec, invalid};
...
impl Runtime {
    ...
    fn start_inner(&mut self, fail: Option<Stage>) -> Result<()> {
        ...
        if self.artifact
            != inferfabric_compiler::compile(&self.artifact.spec, &self.artifact.target)?
...
mod tests {
    ...
    fn artifact() -> Artifact {
        let spec = inferfabric_dsl::parse(include_str!("../../../examples/qwen35-2b.inferfabric"))
            .unwrap();
        inferfabric_compiler::compile(&spec, &Target::mock()).unwrap()
    }
    ...
    fn ascend_fails_closed() {
        ...
        let a = inferfabric_compiler::compile(&a.spec, &target).unwrap();
        let plan = a.ranks[0].clone();
        let mut r = Runtime::new(
            Box::new(AscendBackend),
            a,
    ...
    fn cached_binary_still_captures_each_start_and_rejects_aliases() {
        ...
        let decoded =
            inferfabric_compiler::decode(&inferfabric_compiler::encode(&original).unwrap())
                .unwrap();
        let log = Arc::new(Mutex::new(vec![]));
